...
{
    /// <summary>
    /// A type of quotes calculator specific to "Electricity product" only.
    /// </summary>
    public class ElectricityQuoteCalculator : IQuotesCalculator
    {
        /// <summary>
        /// Returns the annual cost of a plan based on usage
        /// </summary>
        /// <param name="plan">Plan to determine annual cost</param>
        /// <param name="quoteParameter">User provided data such as usage</param>
        /// <returns>Total cost for the year</returns>
        public virtual async Task<double> GetAnnualCostAsync(EnergyPlan plan, QuoteParameter quoteParameter)
        {
            ...
            if(plan.InitialRateExpirationDate == null || !plan.ElectricityFinalUnitRate.HasValue) return await GetElectricityUnitRateAsync(plan, quoteParameter);
            ...
            {
                double initialRateCost = await GetElectricityUnitRateAsync(plan, quoteParameter);
                double finalRateCost = await GetElectricityUnitRateAsync(plan, quoteParameter, true);
            ...
            if (plan.InitialRateExpirationDate != null && remainingDaysBeforeExpiration.HasValue && remainingDaysBeforeExpiration.Value.TotalDays <= 0)
                return await GetElectricityUnitRateAsync(plan, quoteParameter, true);

            return default(double);
        }

        private async Task<double> GetElectricityUnitRateAsync(EnergyPlan plan, QuoteParameter quoteParameter, bool isFinalRate = false)
        {
            return await Task.Run(() =>
            {
                ...
                if(!daysOnInitialUnitRate.HasValue)
                {
                    ...
                    unitRate = quoteParameter.ElectricityUsage * plan.ElectricityInitialUnitRate;
                }
                else
                {
                    ...
                    else
                    {
                        if (!isFinalRate)
                        {
                            unitRate = ((daysOnInitialUnitRate.Value.TotalDays / daysInYear) * quoteParameter.ElectricityUsage) * plan.ElectricityInitialUnitRate;
                        }
                        else
                        {
                            unitRate = GetFinalUnitRate(plan, quoteParameter, daysInYear, daysOnInitialUnitRate);
        ...
        private static double GetFinalUnitRate(EnergyPlan plan, QuoteParameter quoteParameter, int daysInYear, TimeSpan? daysOnInitialUnitRate)
        {
            ...
            return plan.ElectricityFinalUnitRate.CalculateFinalUnitRate(quoteParameter.ElectricityUsage, daysOnFinalUnitRate);
        }
    }
}
